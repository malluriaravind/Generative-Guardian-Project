...
const DataForm: React.FC<DataFormProps> = ({ control, current }: DataFormProps) => <>
    <Controller
        ...
        defaultValue={current?.mistral?.api_key}
    ...
    <TimeoutField
        name='mistral'
        control={control}
        defaultValue={current?.mistral?.timeout}
    />
    <NumberField
        name="mistral.max_connections"
        control={control}
        defaultValue={current?.mistral?.max_connections || 50}
        label="Maximum number of concurrent connections"
    />
    <NumberField
        name="mistral.max_keepalive_connections"
        control={control}
        defaultValue={current?.mistral?.max_keepalive_connections || 10}
