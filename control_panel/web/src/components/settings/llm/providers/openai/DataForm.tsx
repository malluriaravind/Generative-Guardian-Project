...
const DataForm: React.FC<DataFormProps> = ({ control, current }: DataFormProps) => <>
    <Controller
        name="openai.api_key"
        defaultValue={current?.openai?.api_key}
    ...
    <TimeoutField
        name='openai'
        control={control}
        defaultValue={current?.openai?.timeout}
    />
    <NumberField
        name="openai.max_connections"
        control={control}
        defaultValue={current?.openai?.max_connections || 50}
        label="Maximum number of concurrent connections"
    />
    <NumberField
        name="openai.max_keepalive_connections"
        control={control}
        defaultValue={current?.openai?.max_keepalive_connections || 10}
